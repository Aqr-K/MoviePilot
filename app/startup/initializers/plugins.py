...
def _save_plugin_instance_record(instance: PluginInstance) -> None:
    ...
    默认调用目标置位、启用位与版本绑定随描述一起写：读取口
    ``_plugin_instance_from_record`` 会把它们投影出来，读改写一轮下来原值原样回去，
    不存在被顺手抹掉的风险；漏写反而会让「改个展示名」这种无关写入把用户选定的调用
    目标悄悄清掉、把新建的实例落成停用（装载判据正是启用位，实例会建出来却永不加载），
    或把钉住的版本静默改回跟随当前版本。真正的置位、启停与版本切换仍走各自的专用端口，
    这里只负责不丢值。
