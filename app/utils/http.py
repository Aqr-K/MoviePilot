...
def _has_replayable_body(kwargs: dict) -> bool:
    """
    判断请求体在降级重试时能否等价重放：str/bytes/dict 等不可变或每次重新编码的
    内容重放等价；迭代器、异步迭代器、文件对象等一次性主体在 h2 首次发送时可能
    已被消耗，重试会发出空或残缺内容，必须排除
    """
    ...
    if data is not None and not isinstance(data, (str, bytes, dict)):
        return False
    # files 可能包含已被读取的文件对象，逐层判断可重放性不可靠，一律不重试
...
class AsyncRequestUtils:
    ...
    async def request(
        ...
        # _h2_proxy_allowed 处注释）；仅对请求体可等价重放的幂等方法做"h2 失败
        # 就地降级 h1 重试"，避免非幂等请求在服务端可能已收到数据的情况下重复
        # 产生副作用，或一次性请求体被消耗后重试发出空/残缺内容
        ...
        if not (http2 and self._proxies and method.upper() in _REQUESTS_RETRY_IDEMPOTENT_METHODS
                and _has_replayable_body(kwargs)):
