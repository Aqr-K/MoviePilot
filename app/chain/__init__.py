...
from app.chain.dispatch import Dispatcher, multicast as dispatch_multicast
...
class ChainBase(RecognitionMixin, MessageProcessingMixin, NotificationMixin,
    ...
    def __init__(self):
        ...
        self.messagequeue = MessageQueueManager(send_callback=dispatch_multicast)
    ...
    def _dispatcher(self) -> Dispatcher:
        """绑定本链当前管理器与错误上报组件的分发器；见 app.chain.dispatch.Dispatcher。"""
        return Dispatcher(
    ...
    def broadcast(self, method: str, *args, **kwargs) -> None:
        """通知全体提供者，不收集答案；见 app.chain.dispatch.Dispatcher.broadcast。"""
        return self._dispatcher.broadcast(method, *args, **kwargs)

    def multicast(self, method: str, *args, **kwargs) -> List[Any]:
        """收集族类内全部提供者的答案；见 app.chain.dispatch.Dispatcher.multicast。"""
        return self._dispatcher.multicast(method, *args, **kwargs)

    def unicast(self, method: str, *args, **kwargs) -> Any:
        """族类内仲裁取单一答案；见 app.chain.dispatch.Dispatcher.unicast。"""
        return self._dispatcher.unicast(method, *args, **kwargs)

    async def async_broadcast(self, method: str, *args, **kwargs) -> None:
        """异步通知全体提供者，不收集答案；见 app.chain.dispatch.Dispatcher.async_broadcast。"""
        return await self._dispatcher.async_broadcast(method, *args, **kwargs)

    async def async_multicast(self, method: str, *args, **kwargs) -> List[Any]:
        """异步收集族类内全部提供者的答案；见 app.chain.dispatch.Dispatcher.async_multicast。"""
        return await self._dispatcher.async_multicast(method, *args, **kwargs)

    async def async_unicast(self, method: str, *args, **kwargs) -> Any:
        """异步族类内仲裁取单一答案；见 app.chain.dispatch.Dispatcher.async_unicast。"""
        return await self._dispatcher.async_unicast(method, *args, **kwargs)

    def run_module(self, method: str, *args, **kwargs) -> Any:
        """插件优先的模块聚合执行；见 app.chain.dispatch.Dispatcher.run_module。"""
        return self._dispatcher.run_module(method, *args, **kwargs)

    async def async_run_module(self, method: str, *args, **kwargs) -> Any:
        """异步插件优先的模块聚合执行；见 app.chain.dispatch.Dispatcher.async_run_module。"""
        return await self._dispatcher.async_run_module(method, *args, **kwargs)
