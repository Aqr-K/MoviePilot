...
**worker 进程只能以独立脚本方式启动**（文件系统代理用
`subprocess.Popen([sys.executable, <本文件绝对路径>])` 拉起），不能用 `-m` 运行。
直接执行文件路径不会触发 `app/__init__.py` 的导入链，因此这个进程只依赖标准库、
启动是毫秒级的；一旦把它当模块加载就会把整个应用的依赖拉进来，代理被强杀后的
重启成本会高到无法接受。
