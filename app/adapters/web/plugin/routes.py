...
class FastAPIDynamicRouteRegistry:
    ...
    def __init__(
        ...
        log: Any,
        owner_matches: Optional[Callable[[str, str], bool]] = None,
    ) -> None:
        """注入应用、插件投影、认证依赖、日志端口与注册来源匹配规则。"""
        self._app = app
        ...
        self._logger = log
        self._owner_matches = owner_matches or (lambda owner, target: owner == target)
    ...
    def remove(self, plugin_id: str) -> bool:
        ...
        for route in self._app.routes:
            ...
            if self._owner_matches(owner, plugin_id):
                routes.append(route)
        removed = False
        for route in routes:
            try:
