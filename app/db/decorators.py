...
def _is_replaceable_legacy_db_slot(value: Any) -> bool:
    """判断旧 Model 方法签名里 db 位置的既有值是否可以被兼容会话直接覆盖。

    走到 ``_inject_legacy_db`` 说明 ``_get_args_db``/``_get_args_async_db`` 已经确认
    该位置没有匹配当前装饰器类型的会话——但 ``DbOper._db`` 允许在同一实例上混用
    同步/异步方法（见 app/db/base.py 的 ``_execute_async_query`` 等），调用方可能把
    类型不匹配的会话（异步方法收到同步 Session，或反过来）原样传了进来。这类值
    必须当成「未提供」覆盖掉，而不是当成已占位的业务参数在旁边插入新参数，
    否则会把后续位置参数全部错位一位。
    """
...
def _inject_legacy_db(
    ...
    if db_index < len(call_args) and _is_replaceable_legacy_db_slot(call_args[db_index]):
