...
class ExternalIdentity(Base):
    ...
    - SSO 重定向步（OAuth2/OIDC 授权码重定向，``IAuthProvider``，见 ``app/core/auth/redirect.py``）；
    - 「外部直验」凭据步（如对接外部目录/账号系统，``CredentialProviderStep``，
      经 ``app/service/auth/provisioning.resolve_or_create`` 落绑定）。

    绑定以 ``(provider_id, subject)`` 为业务主键：``provider_id`` 标识外部来源
    （SSO 提供方或外部凭据提供方的注册 id），``subject`` 是该来源对主体的
