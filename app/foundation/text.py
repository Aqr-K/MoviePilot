...
if is_free_threaded_runtime():
    ...
    import zhconv_rs  # pylint: disable=no-name-in-module
...
def cut(text: str, HMM: bool = True, cut_all: bool = False) -> list[str]:
    """
    通过统一原生入口执行中文分词，并兼容 jieba.cut 的常用参数名。

    :param text: 待分词文本。
    :param HMM: 是否启用隐马尔可夫模型识别未登录词。
    :param cut_all: 是否使用全模式分词。
    :return: 分词结果列表。
    """
...
def convert(text: str, target: str) -> str:
    """
    执行中文简繁转换，并隔离标准运行时与 free-threaded 运行时的实现差异。

    :param text: 待转换文本。
    :param target: 目标区域代码，例如 ``zh-cn``、``zh-tw``。
    :return: 转换后的文本。
    """
