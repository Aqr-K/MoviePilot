...
_EVENT_STOP_SENTINEL = object()  # 仅用于唤醒消费者退出的队列哨兵
...
class Event:
    ...
    def __init__(self, event_type: Union[EventType, ChainEventType, str],
        ...
        self.correlation_id = correlation_id or get_correlation_id()  # 关联 ID
...
class EventManager(metaclass=Singleton):
    ...
    def __init__(self):
        ...
        # 广播异步处理器的生命周期由事件总线自己持有，避免关闭后仍向主循环投递
    ...
    @staticmethod
    def __join_consumer_threads(consumer_threads: tuple) -> None:
    ...
    def __register_async_handle(self, coroutine: Any) -> bool:
