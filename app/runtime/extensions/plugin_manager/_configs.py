...
class _PluginConfigMixin:
    ...
    async def async_save_plugin_config(self, pid: str, conf: dict, force: bool = False,
                                       instance_id: str = DEFAULT_INSTANCE_ID) -> bool:
        """
        异步保存插件配置
        :param pid: 插件ID
    ...
    def delete_plugin_data(self, pid: str, force: bool = False,
        ...
        if normalized is None:
            ...
            for stored in self._stored_instance_ids(pid) or [DEFAULT_INSTANCE_ID]:
                self._drop_plugin_database(pid, stored)
            return True
        PluginDataOper().del_data(pid, instance_id=normalized)
        self._drop_plugin_database(pid, normalized)
        return True

    def _dispose_plugin_database(self, plugin_id: str,
                                 instance_id: str = DEFAULT_INSTANCE_ID) -> None:
        """
        实例下线后释放其自管理数据库连接

        每个实例各持一个库，释放一个实例的连接不影响其余实例。
        ...
        try:
            ...
            db_manager.dispose(plugin_id, instance_id)
        except Exception as err:
            logger.error(f"释放插件 {plugin_id} 自管理数据库连接出错：{str(err)}")

    @staticmethod
    def _drop_plugin_database(pid: str, instance_id: str = DEFAULT_INSTANCE_ID) -> None:
        """
        删除插件实例自管理的独立库，失败不影响已完成的数据清理
        ...
        try:
            ...
            teardown_plugin_database(pid, instance_id)
        except Exception as err:
