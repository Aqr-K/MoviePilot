    ...
    def __init__(
        ...
        *args: Any,
        message_callback: Optional[
            Callable[[_SchemaMessage], Optional[Awaitable[None]]]
        ] = None,
        **kwargs: Any,
    ) -> None:
...
async def _prepare_web_agent_audio_attachment_path_async(voice_path: str) -> Path:
    """
    异步将 Agent 语音回复准备成 Web 面板可稳定播放的音频文件。

    部分 TTS provider 会生成 Opus/Ogg，桌面 Chromium 通常可播放，但 iOS/Safari
    兼容性不稳定；WebAgent 只在浏览器内播放，因此这里单独转成 WAV。转码等待可取消且有超时。
    """
...
def _build_web_agent_message_events(
    ...
    *,
    ...
    :param prepared_audio_path: 已异步转码好的语音附件路径，缺省时回退为原始路径
