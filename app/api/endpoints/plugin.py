...
@router.post(
    "/clone/{plugin_id}",
    summary="创建插件分身（能力已废弃）",
    response_model=schemas.Response[None],
    responses={
        status.HTTP_410_GONE: {
            "model": schemas.Response[None],
            "description": "该能力已废弃",
        }
    },
)
def clone_plugin(
    ...
    """
    拒绝以复制源码目录方式创建插件分身，并返回废弃登记中的迁移指引
    :param plugin_id: 插件ID
    """
    key = "plugin.clone_by_source_copy"
    try:
        guard(key, context=plugin_id)
        detail = get_notice(key).message(plugin_id)
    except DeprecatedFeatureError as e:
        detail = str(e)
...
def uninstall_plugin(
    ...
    # 配置与数据在卸载后保留，重新安装即可复用；如需清空请调用重置接口
    plugin_manager = PluginManager()
    # 从插件文件夹中移除该插件
