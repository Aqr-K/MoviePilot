...
`plugin.version.overview` 使用源插件 ID 查询，返回该插件磁盘上全部已装版本（按版本号
升序，`is_current` 标出版本元信息登记的当前版本），以及该插件全部实例（首项固定是本体
自身，其后是各个分身）各自的 `pinned_version` 与 `running_version`。用某个分身自身的
实例 ID 当作 `plugin_id` 会被拒绝——版本是源插件的属性，归它的源插件管。

`pinned_version` 与 `running_version` 是两件事。前者是落盘的用户选择：为空表示跟随插件
当前版本，非空表示把这个实例钉在某个已装版本上；后者是此刻实际加载到内存里的那一版。
两者会在三种情况下分开：实例没在跑（`running_version` 为空）、钉住的版本目录已经不在
磁盘上（加载器回落到当前版本）、以及一次失败的切换回退到了原版本。排查「为什么改了版本
没生效」时看的是后者。

`plugin.version.set` 使用源插件 ID 加实例 ID 设置绑定：给出 `pinned_version` 即钉住该
版本，为空即改回跟随当前版本。目标版本必须是已安装版本，否则直接拒绝。

切换是一次完整的停止再启动，不是热替换——热替换等于在运行期换掉一个已注册事件、已起
定时任务、可能有在途请求的实例。因此它可能耗时，也可能失败：目标版本起不来时，切换前
实际运行的那一版会被原样重新启动，绑定一并复原，响应里会说明已回退到哪个版本。

两种情况下切换会被直接拒绝，此时绑定与运行态一律不动：该插件正在安装或写入包文件（磁盘
内容正在变，切过去可能落到写了一半的源码上）；本次切换会让该插件的多个实例分处不同版本，
而这个插件的源码写法不支持多版本并存（自引用绝对导入、在宿主共享声明基类上建模）。后者
的具体原因会逐条写进响应消息。

### 插件旧版本目录回收

`plugin.version.recycle` 使用源插件 ID 删除该插件磁盘上不再被任何实例引用、也不在保留
窗口内的已装版本目录。它删的是磁盘上的插件源码，不可撤销；删掉的版本从已装版本列表中
一并消失，此后无法再被钉住，只能重新安装。

服务端自行判定删哪几个，调用方事先并不知道结果，因此响应里 `removed` 给出本次确实删掉
的版本号，`kept` 逐个给出没被删的理由：当前安装版本、被实例引用（某个实例钉住它、或某个
实例此刻正加载着它——包括已停用的分身钉住的版本）、落在按登记时间的最近若干个版本这个
保留窗口内、本次删除失败下次重试。保留窗口默认留最近 2 个，够一次「装错新版本后切回
上一版」。

整次回收会被直接拒绝、什么都不删的情形有三种：该插件仍有未收尾的安装事务或正在写入包
文件；有实例正在启停或切换版本；安装事务状态无从确认。三者都是可重试的，稍后再调即可。

启动收尾时系统会对全部源码插件自动跑一次同样的回收，这个接口是它的手动入口。
