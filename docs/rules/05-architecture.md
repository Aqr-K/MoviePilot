...
| `app/runtime/extensions/plugin/version.py` | Reversible version-number/version-directory naming, `versions.json` installed-version manifest read/write, legacy flat-layout in-place migration, single-version rollback, and resolution of which source directory a plugin or one of its instances must load |
| `app/runtime/extensions/plugin/binding.py` | Read-only installed-version and instance-binding facts (`PluginVersionInventory`) plus the instance-level version switch with its stop/start compensation chain (`PluginVersionBinding`); it owns no persistence and reaches the lifecycle only through injected ports |
