...
| `app/startup/` | Composition root: inject providers/adapters, order initialization and shutdown, decide restart/lifecycle policy | Reusable business rules, adapter implementation details, or subsystem-agnostic orchestration mechanism (belongs to `app/runtime/kernel/`) | `lifecycle/`, `domain_initializer.py`, `cache_initializer.py`, `modules_initializer.py` |
| `app/runtime/kernel/` | Startup/shutdown orchestration mechanism over abstract components and steps: component model, single-step isolation, ordered execution, managed-resource Runtime assembly | Any concrete subsystem import, or knowledge of which components exist and in what order | `lifecycle.py`, `step_runner.py`, `lifecycle_runner.py`, `managed_resource_runtime.py` |
