...
def test_music_subscribe_reuses_search_download_and_finish_flow():
    ...
    with patch.object(SubscribeChain, "_recognize_music_subscribe", return_value=target), \
            ...
            patch("app.application.orchestration._music.get_chain_subscribe_port") as subscribe_oper:
...
def test_music_best_version_persists_downloaded_rule_priority():
    ...
    with patch("app.application.orchestration._music.DownloadChain", return_value=download_chain), \
            patch("app.application.orchestration._music.get_chain_subscribe_port", return_value=subscribe_oper):
...
def test_album_best_version_requires_confirmed_full_coverage():
    ...
    with patch("app.application.orchestration.subscribe.DownloadChain", return_value=download_chain), \
            patch("app.application.orchestration.subscribe.get_chain_subscribe_port", return_value=subscribe_oper), \
...
def test_recording_target_sync_clears_stale_album_track_count():
    ...
    with patch("app.application.orchestration._music.get_chain_subscribe_port", return_value=subscribe_oper):
...
def test_album_target_sync_does_not_clear_stable_track_count():
    ...
    with patch("app.application.orchestration.subscribe.get_chain_subscribe_port", return_value=subscribe_oper):
...
def test_follow_preserves_album_entity_and_track_count():
    ...
    with patch("app.application.orchestration.subscribe.get_chain_subscribe_port", return_value=subscribe_oper), \
            patch("app.application.orchestration.subscribe.get_configured_system_config", return_value=system_config), \
            patch(
...
def test_refresh_enables_music_entry_fetch_when_music_subscribe_exists():
    ...
    with patch("app.application.orchestration.subscribe.get_chain_subscribe_port", return_value=subscribe_oper), \
            patch("app.application.orchestration.subscribe.get_configured_system_config") as system_config, \
