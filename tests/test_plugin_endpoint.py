...
def test_plugin_history_merges_remote_metadata():
    ...
    with (
        ...
        patch("app.api.endpoints.plugin.PluginMarket", return_value=plugin_market),
    ):
...
def test_plugin_history_returns_installed_plugin_when_remote_missing():
    ...
    plugin_market = MagicMock()
    plugin_market.get_local_repo_plugins.return_value = []
    plugin_market.async_get_online_plugins = AsyncMock(return_value=[])
...
def test_plugin_history_uses_installed_repo_without_refreshing_all_markets():
    ...
    plugin_market = MagicMock()
    plugin_market.get_local_repo_plugins.return_value = []
    plugin_market.async_get_plugins_from_market = AsyncMock(return_value=[market_plugin])
    plugin_market.async_get_online_plugins = AsyncMock(return_value=[])
    ...
    assert result.history == {"v1.1.0": "- 新增更新说明"}
    plugin_market.async_get_plugins_from_market.assert_awaited_once_with(
        "https://github.com/demo/plugins", settings.VERSION_FLAG, True
    )
    plugin_market.async_get_online_plugins.assert_not_awaited()
...
def test_plugin_releases_falls_back_to_compatible_base_package(monkeypatch):
    ...
    plugin_market = MagicMock()
    plugin_market.async_get_plugins_from_market = AsyncMock(
    ...
    assert result["latest_version"] == "1.2.3"
    assert plugin_market.async_get_plugins_from_market.await_args_list == [
        (("https://github.com/demo/plugins", settings.VERSION_FLAG, False), {}),
...
def test_delete_plugin_config_can_force_delete_after_plugin_is_stopped():
    ...
    with patch("app.runtime.extensions.plugin_manager.configs.PluginConfigOper") as plugin_config_oper:
        plugin_config_oper.return_value.delete_plugin.return_value = True
        assert manager.delete_plugin_config("DemoPlugin", force=True) is True

    plugin_config_oper.return_value.delete_plugin.assert_called_once_with("DemoPlugin")
    Singleton._instances.pop((PluginManager, (), frozenset()), None)
...
def test_delete_plugin_data_can_force_delete_after_plugin_is_stopped():
    ...
    with patch("app.runtime.extensions.plugin_manager.configs.PluginDataOper") as plugin_data_oper:
