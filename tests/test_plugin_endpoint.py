...
def test_delete_plugin_config_can_force_delete_after_plugin_is_stopped():
    ...
    with patch("app.runtime.extensions.plugin_manager._configs.PluginConfigOper") as plugin_config_oper:
...
def test_delete_plugin_data_can_force_delete_after_plugin_is_stopped():
    ...
    with patch("app.runtime.extensions.plugin_manager._configs.PluginDataOper") as plugin_data_oper, \
            patch.object(type(manager), "_drop_plugin_database", staticmethod(lambda *args, **kwargs: None)):
        plugin_data_oper.return_value.del_data.side_effect = lambda pid, **kwargs: calls.append(pid)
