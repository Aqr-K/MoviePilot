...
与 ``_is_primary_media_file``（在当前媒体上下文中判定是否主要媒体文件）。``_batches`` 是协调器
自己的状态（非 p115 触达点）。
"""
import unittest
...
def _fake_chain(is_media: bool = True) -> SimpleNamespace:
    """伪造 chain：仅暴露协调器 read-through 的两个面 —— eventmanager 与 _is_primary_media_file。"""
    return SimpleNamespace(
        eventmanager=Mock(),
        _is_primary_media_file=lambda fileitem, mediainfo: is_media,
    )
...
def _task(batch_id="b1", path="/dl/a.mkv") -> SimpleNamespace:
    return SimpleNamespace(
        ...
        mediainfo=SimpleNamespace(title_year="Show (2026)", type=MediaType.TV),
    )
