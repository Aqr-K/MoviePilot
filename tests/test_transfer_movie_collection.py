...
def test_conflicting_download_history_recognizes_movie_by_file_meta(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.application.orchestration.transfer.get_chain_transfer_history_port",
        lambda: SimpleNamespace(get_by_type_tmdbid=lambda **kwargs: None),
    )
    monkeypatch.setattr("app.application.orchestration._transfer.get_chain_transfer_history_port", lambda: SimpleNamespace(get_by_type_tmdbid=lambda **kwargs: None))
    monkeypatch.setattr(
...
def test_movie_collection_conflict_only_drops_automatic_media(
    ...
    monkeypatch.setattr("app.application.orchestration._transfer.get_chain_transfer_history_port", lambda: SimpleNamespace(get_by_src=lambda src, storage=None: None))
    monkeypatch.setattr("app.application.orchestration.transfer.get_chain_download_history_port", lambda: history_oper)
    monkeypatch.setattr("app.application.orchestration._transfer.get_chain_download_history_port", lambda: history_oper)
    monkeypatch.setattr(
