...
from app.adapters.storage.alipan import AliPan
from app.adapters.storage.rclone import Rclone
from app.adapters.storage.u115 import U115Pan
...
def test_alipan_download_writes_sanitized_filename(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch(
            "app.adapters.storage.alipan.transfer_process",
            return_value=_noop_progress,
        ),
        patch(
            "app.adapters.storage.alipan.global_vars.is_transfer_stopped",
            return_value=False,
        ),
        patch("app.adapters.storage.alipan.RequestUtils") as request_utils,
    ):
...
def test_u115_download_writes_sanitized_filename(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch(
            "app.adapters.storage.u115.transfer_process",
            return_value=_noop_progress,
        ),
        patch(
            "app.adapters.storage.u115.global_vars.is_transfer_stopped",
            return_value=False,
        ),
    ):
...
def test_rclone_download_uses_sanitized_target_path(tmp_path: Path) -> None:
    ...
    with (
        patch(
            "app.adapters.storage.rclone.transfer_process",
            return_value=_noop_progress,
        ),
        patch("app.adapters.storage.rclone.subprocess.Popen", side_effect=fake_popen),
    ):
