...
def test_alipan_download_writes_sanitized_filename(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch(
            "app.modules.storages.alipan.transfer_process",
            return_value=_noop_progress,
        ),
        patch(
            "app.modules.storages.alipan.global_vars.is_transfer_stopped",
            return_value=False,
        ),
        patch("app.modules.storages.alipan.RequestUtils") as request_utils,
    ):
...
def test_u115_download_writes_sanitized_filename(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch(
            "app.modules.storages.u115.transfer_process",
            return_value=_noop_progress,
        ),
        patch(
            "app.modules.storages.u115.global_vars.is_transfer_stopped",
            return_value=False,
        ),
    ):
...
def test_rclone_download_uses_sanitized_target_path(tmp_path: Path) -> None:
    ...
    with (
        patch(
            "app.modules.storages.rclone.transfer_process",
            return_value=_noop_progress,
        ),
        patch("app.modules.storages.rclone.subprocess.Popen", side_effect=fake_popen),
    ):
