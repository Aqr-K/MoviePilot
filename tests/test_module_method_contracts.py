...
def test_bangumi_capabilities_share_aggregation_across_sync_async_entry() -> None:
    """Bangumi 同步与异步能力必须共享列表合并或首个非空聚合语义。

    bangumi_info 不在列：已登记进 _SOURCE_PREFIXED_LEGACY_METHODS。
    """
    list_methods = (
        "bangumi_calendar",
        "bangumi_credits",
    ...
    value_methods = ("bangumi_person_detail",)
...
def test_refresh_and_tmdb_cache_contracts_match_host_result_shapes() -> None:
    ...
    assert refresh.aggregation is async_refresh.aggregation
    assert refresh.required_parameters == async_refresh.required_parameters
