...
from app.runtime.extensions import service_registry
...
def test_named_service_config_helpers_preserve_enabled_policy(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        service_registry.ServiceConfigHelper,
        "get_configs",
        staticmethod(lambda config_key, _conf_type: configs.get(config_key, [])),
    )
