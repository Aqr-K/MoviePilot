...
def test_stop_during_thread_bootstrap_preserves_runner_cleanup(monkeypatch) -> None:
    ...
    def block_runner(loop: asyncio.AbstractEventLoop | None) -> None:
        """在 runner 绑定事件循环期间挂起，模拟启动窗口内的停止竞态。"""
...
def test_short_typing_task_can_stop_before_first_trigger(monkeypatch) -> None:
...
def test_typing_stop_retains_blocked_owner_until_terminal(monkeypatch) -> None:
...
def test_stop_all_seals_and_drains_typing_owners(monkeypatch) -> None:
