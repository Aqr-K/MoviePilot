...
from app.adapters.storage.proxy import FileSystemProxy, FileSystemTimeout
...
def test_timeout_raises_and_kills_worker(tmp_path, monkeypatch):
    ...
    """
    import app.adapters.storage.proxy as fsproxy_module
...
def test_worker_does_not_import_app_package():
    ...
    """
    worker = Path("app/adapters/storage/worker.py").read_text(encoding="utf-8")
...
def test_worker_runs_standalone_without_app_on_path(tmp_path):
    ...
    """
    worker_path = Path("app/adapters/storage/worker.py").resolve()
