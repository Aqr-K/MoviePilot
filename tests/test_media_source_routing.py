...
def test_system_only_manager_dispatch_skips_plugin_face() -> None:
    """门面分发的system_only模式不得执行插件钩子面。"""
    manager = object.__new__(MediaRecognizeManager)
    manager._is_valid_empty = Mock(return_value=True)
    manager._dispatch_plugin_modules = Mock(return_value="plugin")
    manager._dispatch_system_modules = Mock(return_value="system")

    result = manager.search_medias(meta=MetaBase("test"), source="anilist", system_only=True)

    assert result == "system"
    manager._dispatch_plugin_modules.assert_not_called()
    manager._dispatch_system_modules.assert_called_once()
