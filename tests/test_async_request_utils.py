...
def test_one_shot_content_body_does_not_downgrade(monkeypatch):
    """
    content 传入迭代器等一次性请求体时不做降级重试，避免 h2 首发已消耗主体后 h1 重放空内容。
    """
    calls = []
    monkeypatch.setattr(
        AsyncRequestUtils, "_dispatch_request", _fake_dispatch(calls, fail_when=lambda http2: http2)
    ...
    utils = AsyncRequestUtils(proxies={"https": PROXY})
    result = asyncio.run(utils.request("get", URL, content=iter([b"chunk"])))
...
def test_files_body_does_not_downgrade(monkeypatch):
    """
    files 传入文件对象时不做降级重试，文件可能已被读取无法保证等价重放。
    """
    calls = []
    monkeypatch.setattr(
        AsyncRequestUtils, "_dispatch_request", _fake_dispatch(calls, fail_when=lambda http2: http2)
    ...
    utils = AsyncRequestUtils(proxies={"https": PROXY})
    result = asyncio.run(utils.request("get", URL, files={"file": ("a.txt", b"data")}))
...
def test_replayable_bytes_content_still_downgrades(monkeypatch):
    """
    str/bytes 等可等价重放的请求体不影响降级重试。
    """
    calls = []
    monkeypatch.setattr(
        AsyncRequestUtils, "_dispatch_request", _fake_dispatch(calls, fail_when=lambda http2: http2)
    ...
    utils = AsyncRequestUtils(proxies={"https": PROXY})
    result = asyncio.run(utils.request("get", URL, content=b"payload"))
