...
def test_pipeline_contract_relays_previous_result_through_named_method() -> None:
    """图片补全等管道契约方法应按优先级把上一 provider 结果交给下一 provider。"""
    class ImageModule:
        """在共享媒体结果上追加当前 provider 名称的测试宿主模块。"""

        def __init__(self, name: str, priority: int) -> None:
            """保存 provider 名称与调度优先级。"""
            self._name = name
            self._priority = priority
