...
def test_register_plugin_builds_isolated_sqlite_engine():
    ...
    try:
        ...
        # 不是核心引擎（物理隔离）
        assert bundle.engine is not get_engine()
    finally:
...
def test_plugin_owns_isolated_table_for_read_write():
    ...
    from app.db.engine import get_engine
    from app.db.plugin import build_plugin_base, db_manager
    ...
    try:
        ...
        assert "note" not in inspect(get_engine()).get_table_names()
        # 插件库里确实有该表
...
def test_register_plugin_postgresql_reuses_shared_engine():
    ...
    try:
        ...
        # 底层连接池即核心引擎的连接池（共享）
        assert bundle.engine.pool is get_engine().pool
    finally:
        db_manager.dispose("pg_plugin")
        settings.DB_TYPE = original


def test_register_plugin_postgresql_matches_case_insensitively():
    """DB_TYPE 大小写不应改变分流：PostgreSQL 与 postgresql 必须都走 schema 路由。"""
    from app.db.plugin import db_manager
    from app.runtime.config import settings
    ...
    settings.DB_TYPE = "PostgreSQL"
    try:
        bundle = db_manager.register_plugin("pg_case_plugin")
        assert bundle.schema == "plugin_pg_case_plugin"
        assert bundle.db_path is None
        assert bundle.owns_engine is False
    finally:
        db_manager.dispose("pg_case_plugin")
        settings.DB_TYPE = original
...
def test_postgresql_schema_ddl_statements():
    ...
    from app.db.plugin.registry import _pg_create_schema_ddl, _pg_drop_schema_ddl
...
def test_run_plugin_migrations_applies_alembic_upgrade_on_sqlite():
    """插件经 Alembic env 模板 + 运行器在自己的独立库执行迁移建表。"""
    import shutil
    import tempfile
    ...
    from app.db.plugin import db_manager
    from app.db.plugin.migration import (
...
def test_setup_plugin_database_uses_migrations_when_location_declared():
    ...
    from app.db.plugin.migration import write_plugin_alembic_env
...
def test_setup_plugin_database_creates_pg_schema_before_tables(monkeypatch):
    ...
    回归保护：直接 ``metadata.create_all(bundle.engine)`` 对 PG 的 schema 路由引擎而言
    目标 schema 从未创建，建表必报 ``schema ... does not exist``。
    ...
    from app.db.plugin import build_plugin_base, db_manager, setup_plugin_database
    from app.runtime.config import settings
    ...
    try:
        ...
        # 记录 create_all 的绑定目标（绑到 engine 即绕过建 schema，绑到事务 conn 才正确）
        monkeypatch.setattr(
            Doc.metadata,
            "create_all",
        ...
        # 必须执行了 DDL（绑 engine 建表不会进 begin()/execute）
        assert "execute" in kinds, "PG 路径未执行任何 DDL —— schema 未被创建"
...
def test_setup_plugin_database_delegates_to_create_tables(monkeypatch):
    ...
    会绕过建 schema 逻辑。本测试锁定接线、防止回归到直接 create_all。
    """
    from sqlalchemy import String, inspect
    from sqlalchemy.orm import Mapped, mapped_column
...
def test_plugin_sqlite_engine_shares_core_builder_contract():
    """插件独立库必须与核心库同调建引擎：驱动错误诊断与 journal_mode 一个都不能少。

    回归保护：插件侧曾各自手抄一份池参数，漏掉了 _register_database_error_logging
    与 DB_CONNECT_ARGS——底层驱动报错在插件库上会静默丢失。
    """
    from sqlalchemy import event, text

    from app.db.diagnostics import _log_database_error
    from app.db.plugin import db_manager

    bundle = db_manager.register_plugin("builder_plugin")
    try:
        # 核心的驱动错误诊断已接入插件引擎
        assert event.contains(bundle.engine, "handle_error", _log_database_error)
        # journal_mode 已作为文件级持久属性设定过（无需逐连接设 PRAGMA）
        with bundle.engine.connect() as conn:
            mode = conn.execute(text("PRAGMA journal_mode;")).scalar()
        assert str(mode).lower() in {"wal", "delete"}
    finally:
        db_manager.drop_plugin("builder_plugin")
